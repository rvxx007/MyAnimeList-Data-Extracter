...
const Character = () => {
  ...
  return (
    ...
    <FilterNav dt={characterData} nxobj={[nxtPage, setNxtPage]} tyobj={[type, setType]}/>
    </header>
    ...
    {characterData.length === 0 ? <Loader/>: <section  className="w-full flex justify-center items-center gap-4 flex-wrap">
      ...
      <div className="w-full h-full group hover:text-white hover:bg-black hover:border-2 border-gray-200 my-2 flex flex-col md:flex-row justify-start items-center p-5 gap-10 md:gap-20 rounded-xl shadow-md ">
            <div >
              <h1 className="w-[3rem] text-xl font-sans font-bold"># {items.srno}</h1>
            </div>
            <div  className="w-full md:w-[400px] flex justify-start items-start gap-5">
            <div >
            ...
            <div  className="w-full flex flex-col md:flex-row justify-start items-start gap-5 md:gap-20" >
            <div  className="w-full md:w-[50%] flex flex-col justify-between items-start ">
              <h1 className="font-bold pb-2">Animeography</h1>
              {items.animeography.map((item)=><p key={uuidv1()} className="text-gray-500 pb-2">{item}</p>)}
            </div>
            <div   className="w-full md:w-[50%] flex flex-col justify-between items-start ">
            <h1 className="font-bold pb-2">Mangaography</h1>
              {items.mangaography.map((item)=><p key={uuidv1()} className="text-gray-500 pb-2">{item}</p>)}
            </div>
            </div>
